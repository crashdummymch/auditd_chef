...
RSpec.configure do |c|
  ...
  c.before(:each) do
    allow_any_instance_of(Kernel).to receive(:open).and_call_original
    allow_any_instance_of(Kernel).to receive(:open)
      .with('http://169.254.169.254/latest/meta-data/mac/')
      .and_return('test_value')
    allow_any_instance_of(Kernel).to receive(:open)
      .with('http://169.254.169.254/latest/meta-data/network/interfaces/macs/test_value/vpc-id/')
      .and_return('vpc-85a560e0')
    allow_any_instance_of(Kernel).to receive(:open)
      .with('http://169.254.169.254/latest/meta-data/network/interfaces/macs/test_value/subnet-id/')
      .and_return('subnet-1c6a4634')
    allow_any_instance_of(Kernel).to receive(:open)
      .with('http://169.254.169.254/latest/meta-data/placement/availability-zone/')
